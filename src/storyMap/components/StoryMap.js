...
const StoryMap = props => {
  ...
  React.useEffect(() => {
    ...
    const initializeInsetMap = ({ setInsetMap, mapInsetContainer }) => {
      ...
      insetMap.on('load', () => {
        setInsetMap(insetMap);
        insetMap.resize();
      });
    };

    if (!map) initializeMap({ setMap, mapContainer });
    if (!insetMap) initializeInsetMap({ setInsetMap, mapInsetContainer });
  }, [map, insetMap, config]);
    ...
    if (map && insetMap) {
      ...
      scroller
        ...
        .onStepEnter(response => {
          ...
          insetMap.flyTo(chapter.location);
  ...
  React.useEffect(() => {
    if (map && insetMap && currentChapter) {
      const bounds = new mapboxgl.LngLatBounds();

      currentChapter.location.center.forEach(center => {
  ...
  }, [map, insetMap, currentChapter]);

  React.useEffect(() => {
    if (map && insetMap && initLoad) {
      const bounds = new mapboxgl.LngLatBounds();

      config.chapters.forEach(chapter => {
        chapter.location.center.forEach(center => {
          bounds.extend(center);
        });
      });
